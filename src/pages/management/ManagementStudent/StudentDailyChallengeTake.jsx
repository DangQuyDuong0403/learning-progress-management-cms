...
const SpeakingWithAudioSectionItem = ({ question, index, theme, sectionScore, isViewOnly }) => {
  ...
  const handleFileUpload = async (event) => {
    ...
    if (oversizeFiles.length > 0) {
      const names = oversizeFiles.map(f => f.name).join(', ');
      spaceToast.error('Size limit is 3MB for audio and 50MB for video. Exceeded limit: ' + names);
      event.target.value = '';
      return;
    }
  ...
  useEffect(() => {
    ...
    const getAnswer = () => {
      ...
      // Prefer recorded audio/video (audioUrl), then uploaded files
      // Recorded audio/video should be saved as string, uploaded files as array
      if (audioUrl) {
        return { answer: audioUrl, questionType: 'SPEAKING' };
      }
      if (Array.isArray(uploadedFiles) && uploadedFiles.length > 0) {
  ...
  useEffect(() => {
    if (triggerAutoSave) triggerAutoSave();
  }, [triggerAutoSave, isRecording, audioUrl, uploadedFiles, recordingType, recordingMode, isPlaying, currentTime]);
