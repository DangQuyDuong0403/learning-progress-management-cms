...
const OptionCard = memo(({ option, index, optionEditorConfig, getPlainText, onRemove, onToggleCorrect, onChange, optionEditorsRef, t, isReadOnly }) => {
	...
	return (
		...
		>
			<div style={{ position: 'absolute', top: '16px', left: '16px', width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.95)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '18px', color: '#333', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', border: `2px solid ${option.color}` }}>
			...
			<div style={{ position: 'absolute', top: '16px', right: '16px', display: 'flex', gap: '8px', alignItems: 'center' }}>
				...
				<Tooltip title={t('dailyChallenge.deleteOption', 'Delete Option')}>
					<Button size="small" danger icon={<DeleteOutlined />} onClick={(e) => { e.stopPropagation(); onRemove(option.id); }} disabled={isReadOnly} style={{ background: 'rgba(255, 77, 79, 0.9)', color: 'white', border: 'none', borderRadius: '8px', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }} />
				</Tooltip>
			</div>
			...
			<div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', marginTop: '50px', marginBottom: '8px', position: 'relative', zIndex: 1 }}>
				<div className={`option-editor option-editor-${option.id}`} style={{ borderRadius: '12px', overflow: 'hidden', background: 'rgba(255, 255, 255, 0.98)', border: '2px solid rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', boxShadow: '0 2px 12px rgba(0, 0, 0, 0.08)' }}>
					<CKEditor key={`option-editor-${option.id}`} editor={ClassicEditor} data={option.text} config={optionEditorConfig} onChange={(event, editor) => onChange(option.id, event, editor)} onReady={(editor) => { optionEditorsRef.current[option.id] = editor; }} />
				</div>
...
// hover handled locally in OptionCard
